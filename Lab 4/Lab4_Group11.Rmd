...
And finally, once we have the posterior, we can compute the the probability distribution $p(\mu_{i}|\vec{\mu}_{i},\vec{Y})$, where $\mu_{-i}$ is a vector containing all $\mu$ values except for $\mu_{i}$. Using Hint C, we get to the following result:
