...
In \ref{fig:gibbs} there is a plot of the dependence of \texttt{Y} on \texttt{X} and the comparison with the expected values of mu.   
...
The result is considered as positive as observing the plot it is possible to see that the expected values of $\mu$ follow the same pattern that \texttt{Y} in terms of dependence on \texttt{X}. Also, the expected values seem to have removed the noise produced by the accuracy of the instrumentat used to measure, as the expected values of $mu$ are less spread than those of \texttt{Y}. 
