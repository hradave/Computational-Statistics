...
```{r}
log_likelihood <- function(X, lambda) {
  return sum(X/(2*lambda))
}

EM_algorithm <- function(Z, lambda, eps, k_max) {
    Z_obs <- Z[!is.na(Z)]
    Z_miss <- Z[is.na(Z)]
    ...
    ll_prev<-log_likelihood(Z_obs,lambda)
    ll_curr<-ll_prev+10+100*eps
    
    while ((abs(lambda_prev-lambda_curr)>=eps) && (k<(kmax+1))){
	    llvalprev<-llvalcurr
	    ## E-step
	    EY<-sum(Z_obs/(2*lambda_old))
      #EY <- sum(X/(2*lambda_old))
