...
>>>>>>> c9fa62c70d1a6f8d952d57068a49c3b4a5e3e2c0
```
...
EM_algorithm <- function(X, Y, Z, lambda, eps, k_max) {
    ...
    while (k<k_max+1 && abs(lambda_prev-lambda_curr)>=eps) {
	    ...
	    ## E-step
	    EY <- -n*log(2*lambda_prev^2) + sum(log(X^2)) - sum(X_obs*(Z_obs+2*Y_obs))/(2*lambda_prev) - sum(lambda_curr+X_miss*Y_miss)/lambda_prev 
...
<<<<<<< HEAD
As we can see in just 5 iterations we reached the optimal $\lambda$. 
